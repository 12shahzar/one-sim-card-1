...
const CoverageSection = () => {
  ...
  const handleCountryClick = (country) => {
    console.log(country); // log the full country data
  ...
  return (
    ...
    min-h-[calc(100vh-3rem)] /* same for top + bottom */
      ...
      <section className="min-w-7xl mx-auto bg-white rounded-4xl p-8 sm:p-10 font-sora border border-[#D2D2D2] mb-4 md:mb-20">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <p className="text-[#08080C] text-xl font-regular ">
            Use the tool below to discover the countries in which <br></br>and networks
            on which we operate.
          </p>
        ...
        <div className="mt-8 overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#F5F5F5] text-[#08080C] font-bold text-base ">
                <th className="py-6 px-4 rounded-l-2xl">Mobile Carrier</th>
                <th className="py-6 px-4">Code</th>
                <th className="py-6 px-4 rounded-r-2xl">Speed</th>
              </tr>
            </thead>
            <tbody>
              {(coverageData[selectedCountry] || generalCoverageData).map(
                (item, index) => (
                  <tr
                    key={index}
                    className="border-b border-[#D2D2D2] text-[#6B7280] text-base font-regular"
                  >
                    <td className="py-6 px-4">{item.carrier}</td>
                    <td className="py-6 px-4">{item.code}</td>
                    <td className="py-6 px-4">{item.speed}</td>
                  </tr>
                )
              )}
            </tbody>
